...
public class TruckController {
    ...
    public ResponseEntity<List<String>> getAllBrands() {
        ...
        List<String> brandNames = brands != null
                ? List.of(brands).stream().map(FipeBrandDTO::getName).collect(Collectors.toList())
                : List.of();
        return new ResponseEntity<>(brandNames, HttpStatus.OK);
    }

    @GetMapping("/fipe/brands/{brandCode}/models")
    public ResponseEntity<List<String>> getModelsByBrand(@PathVariable String brandCode) {
        FipeModelResponseDTO response = fipeApiClient.getModelsByBrand(brandCode);
        List<String> models = response != null && response.getModels() != null
                ? response.getModels().stream().map(m -> m.getName()).collect(Collectors.toList())
                : List.of();
        return new ResponseEntity<>(models, HttpStatus.OK);
    }

    @GetMapping("/fipe/brands/{brandCode}/models/{modelCode}/years")
    public ResponseEntity<List<String>> getYearsByModel(
            @PathVariable String brandCode,
            @PathVariable String modelCode
    ) {
        List<FipeYearDTO> years = fipeApiClient.getYearsByModel(brandCode, modelCode);
        List<String> yearCodes = years != null
                ? years.stream().map(FipeYearDTO::getCode).collect(Collectors.toList())
                : List.of();
        return new ResponseEntity<>(yearCodes, HttpStatus.OK);
    }

    @GetMapping("/fipe/brands/{brandCode}/models/{modelCode}/years/{yearCode}")
    public ResponseEntity<FipePriceResponseDTO> getPriceFromFipe(
            @PathVariable String brandCode,
            @PathVariable String modelCode,
